...
export const AiMLchartData= async () => {
  try {
    const response = await api.get('/ai_ml_chartdata');
    return response.data;
  } catch (error) { 
    console.error('Error fetching hello data:', error);
    throw error;
...
export const fetchUserData = async () => {
  try {
    const response = await api.get('/api/user-data'); // Token will be added automatically
    return response.data;
  } catch (error) {
...
export const uploadAudioApi = async (file) => {
  ...
  try {
    const response = await api.post('/upload_audio_file', formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
// Sign-in function (login request)
export const signIn = async (email, password, company) => {
  try {
