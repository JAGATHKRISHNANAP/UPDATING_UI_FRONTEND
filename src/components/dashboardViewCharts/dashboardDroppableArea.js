    ...
    {chartdata && chartdata.length > 0 ? (
      chartdata.map((chart, index) => (
        <div
          key={index}
          style={{
            position: 'absolute', // Position each chart absolutely
            left: chart.position?.x || 0, // Set X position
            top: chart.position?.y || 0, // Set Y position
            zIndex: 1 // Ensure charts stack properly
          }}
        >
          {chart.chart_type === 'bar' && (
            <BarChart
